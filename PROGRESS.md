...
Last updated: Phase 15 complete (Expo mobile app).
...
- **Phase 15 (mobile app):** Expo app at `pgfishipping/mobile/` (Expo SDK 54, Router v6) with `axios` + `zustand` + `expo-secure-store` for persisted JWT session and shared API base `EXPO_PUBLIC_API_URL` defaulting to `http://localhost:4000/api`. **Auth** — `(auth)/login|register|forgot-password` and redirect gate on `(tabs)`. **Home** — `GET /user/me` welcome line + `GET /wallet/balance` balance + Expo push token (local permission only; no server registration yet — no DB model). **Shipments** — tab filters All / Pre-alerts / Active / Done (maps to `status` on `GET /shipments`); list links to `shipment/[id]/index` for detail (`GET /shipments/:id` with event timeline) + “Pay from wallet” (`POST /wallet/pay-shipment`) + link to `shipment/[id]/camera` for **invoice** capture (`expo-camera` + multipart `POST /shipments/:id/invoice`). **Wallet** — `GET /wallet/balance` with transactions, deposit via `POST /wallet/deposit` + `expo-web-browser` on `redirectUrl`, gift-card redeem `POST /wallet/redeem-gift-card`. **Calculator** — `POST /calculator/estimate` with line-item fee breakdown, AIR/SEA toggle, optional FOB. **Profile** — logout. **Quality** — `npm run typecheck` (`tsc --noEmit`) clean; `app.json` name/slug `pgfishipping-mobile`. Root `README` documents `expo start`. Stray root `package-lock.json` (accidental) removed.
